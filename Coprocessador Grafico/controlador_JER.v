module controlador_JER (
   ...
   // ========= Multiplexador de Coordenadas para ROM - EXPANDIDO =========
   wire [9:0] x_rom_select, y_rom_select;
   ...
   assign x_rom_select = usar_media_blocos ? x_atual : x_fonte0;
   assign y_rom_select = usar_media_blocos ? y_atual : y_fonte0;

   // ========= Validação de Coordenadas =========
   wire [9:0] x_fonte_valido, y_fonte_valido;
   assign x_fonte_valido = (x_rom_select < ORIG_WIDTH) ? x_rom_select : (ORIG_WIDTH - 1);
   assign y_fonte_valido = (y_rom_select < ORIG_HEIGHT) ? y_rom_select : (ORIG_HEIGHT - 1);

   // ========= Endereço ROM =========
   reg [14:0] endereco_rom;
   always @(posedge clock_25MHz) begin
      if (reset) begin
         endereco_rom <= 0;
      end else begin
         endereco_rom <= y_fonte_valido * ORIG_WIDTH + x_fonte_valido;
      end
   end
      ...
      if (reset || config_mudou) begin
         ...
         soma_pixels <= 16'd0;
            ...
            case (estado_media)
               ...
               5'd1: begin // Pixel 0
                  soma_pixels <= soma_pixels + {8'b00000000, pixel_rom_delayed};
                  estado_media <= 5'd2;
               end
               5'd2: begin // Pixel 1
                  soma_pixels <= soma_pixels + {8'b00000000, pixel_rom_delayed};
   ...
   // ========= Pipeline para ROM e Cálculo de Média - EXPANDIDO =========
